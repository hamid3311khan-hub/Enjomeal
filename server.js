...
app.post('/api/rider-register', uploadRider.fields([{name: 'aadharImg'}, {name: 'panImg'}, {name: 'photoImg'}]), async (req,res)=>{
  try{
    ...
    if(existing) return res.json({success:false, msg:"Mobile pehle se registered hai"});
    const uploadFile = async (file) => { if(!file) return ""; const result = await cloudinary.uploader.upload(`data:${file[0].mimetype};base64,${file[0].buffer.toString('base64')}`, {folder: 'eat4bite_riders'}); return result.secure_url; };
    const aadharUrl = await uploadFile(req.files['aadharImg']);
    const panUrl = await uploadFile(req.files['panImg']);
    const photoUrl = await uploadFile(req.files['photoImg']);
...
app.get('/invoice', async (req,res)=>{
  ...
  doc.pipe(res);
  doc.fontSize(20).text('Eat4Bite™ Invoice', {align: 'center'});
  doc.moveDown();
  doc.fontSize(12).text(`Order ID: ${order.trackId}`);
  doc.text(`Name: ${order.name}`);
  doc.text(`Phone: ${order.phone}`);
  doc.text(`Address: ${order.address}`);
  doc.text(`Date: ${new Date(order.createdAt).toLocaleString('hi-IN')}`);
  doc.moveDown();
  order.items.forEach(item => {
    doc.text(`${item.name} x ${item.qty} - ₹${item.price * item.qty}`);
  });
  doc.moveDown();
  doc.text(`Item Total: ₹${order.item_total}`);
  doc.text(`Platform Fee: ₹${order.platform_fee}`);
  doc.text(`Delivery: ₹${order.delivery_fee}`);
  doc.text(`Commission 5%: ₹${order.commission_5}`);
  doc.fontSize(14).text(`Grand Total: ₹${order.grand_total}`);
  doc.end();
});
...
app.get('/admin-restaurants', (req,res)=> res.sendFile(path.join(__dirname, 'public', 'admin-restaurants.html')));
