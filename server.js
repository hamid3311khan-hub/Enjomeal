...
const uploadRider = multer({ storage: multer.memoryStorage() }); // ADD KIYA
...
.then(()=>console.log('✅ MongoDB Connected v3.9.0 - Rider Added'))
...
const Rider = mongoose.model('Rider', {
  ...
  lat:Number, lng:Number, lastUpdate:Date,
  status:{type:String, default:"Pending"},
...
app.post('/api/admin/assign-rider', async (req,res)=>{
  try{
    ...
    io.emit('newOrderForRider', {riderId}); // FIXED
...
app.get('/api/admin/rider-cash', async (req,res)=>{ try{
  const riders = await Rider.find({status: "Approved"});
  res.json(riders.map(r=>({name:r.name, mobile:r.mobile, pending_cash:r.cash_balance})))
}catch(e){ res.json([]) }});

app.post('/api/admin/rider-deposit', async (req,res)=>{ try{
  await Rider.updateOne({mobile: req.body.mobile}, {$set: {cash_balance: 0}});
...
app.post('/api/broadcast', async (req,res)=>{ try{
  ...
  owners.forEach(o=> sendWhatsApp(o.mobile, `📢 ADMIN NOTICE: ${req.body.message}`));
...
app.post('/api/rider-register', uploadRider.fields([{name: 'aadharImg'}, {name: 'panImg'}, {name: 'photoImg'}]), async (req,res)=>{
  try{
    ...
    if(await Rider.findOne({mobile})) return res.json({success:false, msg:"Mobile pehle se registered hai"});
    const upload = async (file) => { if(!file) return ""; const result = await cloudinary.uploader.upload(`data:${file[0].mimetype};base64,${file[0].buffer.toString('base64')}`, {folder: 'eat4bite_riders'}); return result.secure_url; }
    const aadharUrl = await upload(req.files['aadharImg']); const panUrl = await upload(req.files['panImg']); const photoUrl = await upload(req.files['photoImg']);
    await new Rider({restaurantId, name, fatherName, aadhar, pan, mobile, aadharImg: aadharUrl, panImg: panUrl, photoImg: photoUrl}).save();
    res.json({success:true, msg:"Registered! Admin approval ka wait kare"})
  }catch(e){ res.json({success:false, msg:e.message}) }
});

app.post('/api/rider-login', async (req,res)=>{
  try{ const rider = await Rider.findOne({mobile: req.body.mobile}); if(!rider) return res.json({success:false, msg:"Rider nahi mila"}); if(rider.status === "Pending") return res.json({success:false, msg:"Approval pending hai"}); res.json({success:true, rider}) }catch(e){ res.json({success:false, msg:"Server Error"}) }
});
...
app.post('/api/restaurant-register', upload.fields([{ name: 'restaurantImage', maxCount: 1 }, { name: 'paymentProof', maxCount: 1 }]), async (req,res)=>{
  try{
    ...
    let restaurantImageUrl = ""; let paymentProofUrl = "";
    if(req.files['restaurantImage']){ const result = await cloudinary.uploader.upload(`data:${req.files['restaurantImage'][0].mimetype};base64,${req.files['restaurantImage'][0].buffer.toString('base64')}`, {folder: 'eat4bite_restaurants'}); restaurantImageUrl = result.secure_url; }
    if(req.files['paymentProof']){ const result = await cloudinary.uploader.upload(`data:${req.files['paymentProof'][0].mimetype};base64,${req.files['paymentProof'][0].buffer.toString('base64')}`, {folder: 'eat4bite_proofs'}); paymentProofUrl = result.secure_url; }
    const restaurantId = "shop_" + Date.now(); const regAmount = Number(amount) || 200; const planType = regAmount === 2000? "Annual" : "Trial"; const trialDays = regAmount === 2000? 365 : 7; const trialEnd = new Date(); trialEnd.setDate(trialEnd.getDate() + trialDays);
    await new RestaurantOwner({ restaurantId, restaurantName, ownerName, mobile, email, address, password, status: "Pending", image: restaurantImageUrl, payment_proof: paymentProofUrl, trial_end_date: trialEnd, plan_status: planType, registration_fee_paid: regAmount, upi_id: "tanbalkhi2014-3@okhdfcbank" }).save();
    res.json({ success:true, msg:"Registration ho gayi. Approval ke baad login karna", upi_id: "tanbalkhi2014-3@okhdfcbank" })
