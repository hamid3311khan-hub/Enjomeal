...
// ===== NEW DAILY PAYOUT API =====
app.get('/api/admin/daily-payout', async (req, res) => {
    try {
        const today = new Date().toISOString().split('T')[0];
        const startOfDay = new Date(today + 'T00:00:00.000Z');
        const endOfDay = new Date(today + 'T23:59:59.999Z');
        const orders = await Order.find({ createdAt: { $gte: startOfDay, $lte: endOfDay }, status: { $in: ['Delivered', 'Paid'] } });
        let totalSales = 0; let platformFee = 0; let toPay = 0; let result = [];
        orders.forEach(o => {
            const sale = Number(o.grand_total || o.total || 0);
            const fee = sale * 0.10; const pay = sale - fee;
            totalSales += sale; platformFee += fee; toPay += pay;
            result.push({ orderId: o.trackId, restaurant: o.restaurantId, amount: sale, fee: fee, toPay: pay, status: o.status });
        });
        res.json({date: today, totalOrders: orders.length, totalSales, platformFee, toPay, orders: result});
...
app.post('/api/admin/mark-paid', async (req, res) => {
    try {
        ...
        await Order.updateMany({ trackId: { $in: orderIds } }, { $set: { status: 'Paid' } });
        res.json({ success: true, message: 'Marked as Paid' });
    } catch (e) { res.status(500).json({ message: e.message }); }
});

// ===== RIDER APIs - FIXED HERE =====
app.post('/api/rider-register', uploadRider.fields([{name: 'aadharImg'}, {name: 'panImg'}, {name: 'photoImg'}]), async (req,res)=>{ try{ const {restaurantId, name, fatherName, aadhar, pan, mobile} = req.body; if(await Rider.findOne({mobile})) return res.json({success:false, msg:"Mobile pehle se registered hai"}); const uploadFile = async (file) => { if(!file) return ""; const result = await cloudinary.uploader.upload(`data:${file[0].mimetype};base64,${file[0].buffer.toString('base64')}`, {folder: 'eat4bite_riders'}); return result.secure_url; } const aadharUrl = await uploadFile(req.files['aadharImg']); const panUrl = await uploadFile(req.files['panImg']); const photoUrl = await uploadFile(req.files['photoImg']); await new Rider({restaurantId, name, fatherName, aadhar, pan, mobile, aadharImg: aadharUrl, panImg: panUrl, photoImg: photoUrl}).save(); res.json({success:true, msg:"Registered! Admin approval ka wait kare"}) }catch(e){ res.json({success:false, msg:e.message}) }});
...
app.post('/api/orders', async (req,res)=>{ try{ const trackId = 'EB' + Date.now(); const item_total = req.body.items.reduce((a,b)=>a+(b.price*b.qty), 0); const bill = calculateBill(item_total); const shop = await RestaurantOwner.findOne({restaurantId: req.body.restaurantId}); const upi_id = shop? shop.upi_id : "tanbalkhi2014-3@okhdfcbank"; const upi_link = `upi://pay?pa=${upi_id}&pn=${shop.restaurantName}&am=${bill.grand_total}&cu=INR&tn=Order${trackId}`; await new Order({...req.body, trackId,...bill}).save(); res.json({success:true, trackId, upi_link}) }catch(e){ res.json({success:false, msg:e.message}) }});

server.listen(PORT, () => console.log(`🚀 Server v3.9.1 DAILY PAYOUT on ${PORT}`));