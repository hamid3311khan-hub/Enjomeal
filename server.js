...
app.post('/api/coupon/validate', async (req,res)=>{
    ...
    if(!offer) return res.json({success:false, msg:"Invalid Coupon"});
    res.json({success:true, discount: offer.discount, type: offer.type});
});

app.post('/api/restaurant/register', async (req,res)=>{
    try{
        const {restaurantId, restaurantName, ownerName, mobile, email, address, password, image_base64, upi_id} = req.body;
        ...
        await new RestaurantOwner({...req.body, password: hashedPassword, image_base64, upi_id, trial_end_date: trialEnd}).save();
...
app.get('/api/rider/ledger', async (req,res)=>{
    ...
    res.json({ pending_cash: rider.cash_balance, limit: rider.cashLimit, today_collected: collected, today_orders: orders.length, progress: Math.round((rider.cash_balance / rider.cashLimit) * 100) })
});
